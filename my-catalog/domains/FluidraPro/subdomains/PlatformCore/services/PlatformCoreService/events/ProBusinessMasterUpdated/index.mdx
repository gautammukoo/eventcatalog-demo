...
name: Pro Business Master Updated
version: 1.0.0
summary: |
  Pro Business Master Updated
owners:
  - full-stack
channels:
...
The `Pro Business Master Updated` event is triggered whenever there is a dealer Onboarded.
...
<Tiles>
  ...
  <Tile
    icon="DocumentIcon"
    href={`/generated/events/ProBusinessMasterUpdated/${frontmatter.id}/schema.avro`}
...
To produce an Pro Business Master Updated event, use the following example Event Bridge producer configuration in Typescript:
...
To consume an Pro Business Master Updated event, use the following example Event Bridge consumer configuration in Typescript:
